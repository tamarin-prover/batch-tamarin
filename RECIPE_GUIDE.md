...
-   **`output`** (object, optional): Output processing configuration

#### Output Configuration (`output`):

-   **`store_raw_output`** (boolean, optional, default: false): Whether to store raw stdout/stderr from tasks for debugging purposes

**Note**: The `--raw` CLI flag or `--debug` flag will override this setting.
