...
class TaskRunner:
    ...
    def __init__(self, global_config: GlobalConfig) -> None:
        ...
        self.output_processor = TamarinOutputProcessor(output_directory)
    ...
    def _handle_task_completion(self, task: ExecutableTask, result: TaskResult) -> None:
        ...
        - Process output to generate result.json
        ...
        try:
            # Find the Tamarin output file (generated by --output flag)
            tamarin_output_file = (
                task.output_file if task.output_file.exists() else None
            )

            # Process the output with lemma filter if specified
            lemma_filter = task.lemma if task.lemma else None

            result_file = self.output_processor.process_task_output(
                result, tamarin_output_file, lemma_filter
            )
            notification_manager.debug(
                f"[TaskRunner] Generated result file for {task_id}: {result_file}"
            )
        except Exception as e:
            notification_manager.error(
                f"[TaskRunner] Failed to process output for {task_id}: {e}"
            )
