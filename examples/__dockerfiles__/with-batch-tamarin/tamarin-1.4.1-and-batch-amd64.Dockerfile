# Tamarin Prover 1.4.1 and batch-tamarin with Maude using NixOS
# AMD64-only due to Tamarin 1.4.1 limitations

FROM --platform=linux/amd64 nixos/nix:latest AS nix-environment
...
COPY tamarin-1.4.1-and-batch.nix /tmp/flake.nix
RUN cd /tmp && nix profile add .#default

# Set environment
ENV PATH="/root/.nix-profile/bin:$PATH" \
    TAMARIN_VERSION=1.4.1
