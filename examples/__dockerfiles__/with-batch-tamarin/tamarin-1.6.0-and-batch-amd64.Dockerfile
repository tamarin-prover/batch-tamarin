# Tamarin Prover 1.6.0 and batch-tamarin with Maude using NixOS
# AMD64-only due to Tamarin 1.6.0 limitations
...
COPY tamarin-1.6.0-and-batch.nix /tmp/flake.nix
RUN cd /tmp && nix profile add .#default

# Set environment
ENV PATH="/root/.nix-profile/bin:$PATH" \
    TAMARIN_VERSION=1.6.0 \
    LC_ALL=C.UTF-8 \
    LANG=C.UTF-8
